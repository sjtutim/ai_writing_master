...
import { downloadFromMinio, uploadToMinio, convertWordToMarkdown, convertPdfToText, splitTextToChunks } from './document';
...
async function processParseJob(job: any): Promise<void> {
  ...
  if (mimetypeLower.includes('word') || filenameLower.endsWith('.docx') || filenameLower.endsWith('.doc')) {
    ...
    markdownContent = await convertPdfToText(fileBuffer);
...
async function processChunkJob(job: any): Promise<void> {
  ...
  const chunks = splitTextToChunks(markdownContent, chunkSize, overlap);
...
async function processEmbedJob(job: any): Promise<void> {
  ...
  // 检查哪些分块还没有向量化
  const chunksWithEmbedding = await prisma.$queryRaw<Array<{ id: string }>>`
  ...
  // 使用配置的批处理参数
  const batchSize = config.chunking.embeddingBatchSize;
  const batchDelay = config.chunking.embeddingBatchDelay;
  ...
  // 只有当所有分块都成功处理时才标记为完成
  // 重新检查是否所有分块都已向量化
  const remainingChunks = await prisma.$queryRaw<Array<{ count: bigint }>>`
